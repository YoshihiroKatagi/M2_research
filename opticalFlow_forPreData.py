...
start_time = 2
end_time = 36
total_time = end_time - start_time # 34
...
# # データシート
# datasheet = data_path + "/datasheet.csv"
#####################################################################
...
def Preprocess(echo_file_name):
  ...
  end_frame = end_time * frame_rate # 4161
...
def OpticalFlow(echo_file_name):
  ...
  size = (width, height)  # (510, 560)
  frame_count = int(video.get(cv2.CAP_PROP_FRAME_COUNT)) # 3285
  ...
  feature_points_of_all = np.delete(feature_points_of_all, np.s_[feature_num:], 1) # (1548, 50, 2)
  feature_points_of_all = feature_points_of_all.reshape([1, feature_points_of_all.shape[0], feature_points_of_all.shape[1] * 2]) # (1, 1548, 100)
...
def SaveProccessedImage(echo_file_name, feature_points, theta):
  ...
  feature_points = feature_points[:, :, :100].reshape([feature_points.shape[1], feature_num, 2]) # (1548, 50, 2)
  video = cv2.VideoCapture(target_path)
  ...
  width = int(video.get(cv2.CAP_PROP_FRAME_WIDTH))
  size = (width, height)  # (510, 560)
  frame_count = int(video.get(cv2.CAP_PROP_FRAME_COUNT))  #1548
  frame_rate = int(video.get(cv2.CAP_PROP_FPS)) # 43
